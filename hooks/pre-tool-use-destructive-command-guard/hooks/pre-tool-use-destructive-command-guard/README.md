...
`Bash`, and uses `${CLAUDE_PROJECT_DIR}` so it still works after Claude changes
...
printf '%s\n' '{"tool_input":{"command":"rm -rf dist"},"cwd":"/workspace/app"}' \
  | hooks/pre-tool-use-destructive-command-guard/.claude/hooks/destructive_command_guard.py
```
